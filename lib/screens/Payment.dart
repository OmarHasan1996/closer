...
*/
  ...
  initState() {
    ...
    _dateController.text =
        DateTime.now().toString().split(' ')[0].split('-')[0].substring(2) +
            ' / ' +
            selectedDate.toString().split(' ')[0].split('-')[1];
    ...
    cardSecurityControler.addListener(() {
      _onlyThreeChar(cardSecurityControler);
    });
    cardNumberControler.addListener(() {
      _cardNumber(cardNumberControler);
    });
...
*/
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              _card(
                  AppLocalizations.of(context)!.translate('cardHolderName'),
                  cardHolderNameControler,
                  '',
              ...
              _bottun(AppLocalizations.of(context)!.translate('Order Now'),
                  _orderNow),
            ],
          ),
        ),
    ...
    return AppBar(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.only(
            bottomRight:
                Radius.circular(MediaQuery.of(context).size.height / 80 * 3),
            bottomLeft:
                Radius.circular(MediaQuery.of(context).size.height / 80 * 3)),
      ),
  ...
  _card(headerText, controller, hintText, width, obscureText, bool num,
      {bool? cardNum}) {
    cardNum ??= false;
    return !cash
        ? Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Expanded(
                      ...
                      child: TextField(
                        keyboardType:
                            num ? TextInputType.number : TextInputType.name,
                        ...
                        onChanged: (k) {
                          if (cardNum!) {
                            ...
                            cardNumberControler.selection =
                                TextSelection.fromPosition(TextPosition(
                                    offset: cardNumberControler.text.length));
                            print(k);
                          }
                          //controller.text=k;
                        },
  ...
  _bottun(text, press) {
    return MyWidget(context).raisedButton(
        text, () => press(), MediaQuery.of(context).size.width / 1.2, chCircle);
    /*return Container(
  ...
  _bayCash() async {
    _bay() async {
      ...
      if (_suc) {
        var fcmBoss = '';
        try{
          List service = _order['Servicess'].toList();
          for(var s in  service){
            var bossGroup = s['Group']['GroupUsers'].where((element) => element['isBoss']==true).toList();
            var _fcmBoss=(bossGroup[0]['Users']['FBKey']??'');
            if(fcmBoss != _fcmBoss){
              fcmBoss = _fcmBoss;
              api!.sendPushMessage(_fcmBoss, AppLocalizations.of(context)!.translate('New Order'),_order['Serial']);
            }
          }
        }catch(e){
        }
        ...
        builder: (context) => AlertDialog(
              content: ListTile(
                ...
                subtitle: Text(AppLocalizations.of(context)!
                    .translate('Are you sure? You want to pay Cash!')),
              ),
              actions: <Widget>[
                IconButton(
                  icon: Icon(
                    Icons.close, /*color: MyColors.yellow*/
                  ),
                  onPressed: () {
                    Navigator.pop(context);
                    //setState(() {});
  ...
  Future<Null> _selectDate(BuildContext context) async {
    ...
    if (picked != null)
      setState(() {
        ...
        _dateController.text =
            selectedDate.toString().split(' ')[0].split('-')[0].substring(2) +
                ' / ' +
                selectedDate.toString().split(' ')[0].split('-')[1];
      });
  }
  ...
  _orderNow() async {
    if (cash) {
      ...
      //await checkout();
      if (cardSecurityControler.text == '' ||
          cardHolderNameControler.text == '' ||
          cardNumberControler.text == '' ||
          _dateController.text == '')
        APIService.flushBar(AppLocalizations.of(context)!
            .translate('Please! Fill all information before Order'));
      else
        await checkoutIyzi();
    }
  }
...
//Set up your iyzico configurations
    ...
    final shippingAddress = iyzi.Address(
        address:
            '${_order['Address']['notes']}, ${_order['Address']['building']}. ${_order['Address']['Floor']}. No:${_order['Address']['appartment']}',
